...
5. **Given** a create body missing `secret`, or with an unsupported `provider`, or a `repo` already registered for this user+provider, **When** posted, **Then** `400` for the missing/invalid field or `409` for the duplicate `(provider, repo)`, and nothing is written.

---

### User Story 3 - Unverified, unmapped, and unsupported deliveries are rejected safely (Priority: P1)

As the owner, I need deliveries that fail signature verification, name an unregistered repository, or target an unsupported provider to be rejected without writing anything and without leaking a secret, a commit, or any other owner data. (A wrong signature on a *registered* repo returns `401` while an unregistered repo returns `404`, so the codes do reveal whether a known-named repo is registered — an accepted, low-impact trade-off; see research D-7.)
...
- **Provider handshake / non-push events**: GitHub sends a `ping` on setup and may send events other than `push`. The event is read from the provider event header (`X-GitHub-Event` / `X-Gitlab-Event`); any present, non-empty value other than the provider's push event (GitHub `push`, GitLab `Push Hook`) — including `ping` — is a *recognized non-`push`* event and is acknowledged with `202` and zero counts (`{ received: 0, ingested: 0, skipped: 0 }`) — accepted, nothing ingested — so the host does not disable the hook. A **missing or empty** event header means the delivery names no event and returns `400` (FR-007), the same as an otherwise unidentifiable body.
...
- **CSRF**: the receiver is a cross-origin POST with no `Authorization` header and no same-origin `Origin`; it must be exempt from the global CSRF guard (PR2 wiring, FR-014), joining the existing exemption for the public email-verification route. (That route — `/api/v1/auth/link/verify/` — is a **GET**; the receiver would be the first CSRF-exempt public *write* POST, which is still safe because its authenticity is the per-repo signature a CSRF attacker cannot forge, not an ambient session cookie.)
...
- **FR-002**: For a recognized `push` delivery, the receiver MUST resolve the delivery's target **registration** from `(provider, repo)`, where `repo` is read from the documented payload (GitHub `repository.full_name`; GitLab `project.path_with_namespace`). If the payload does not carry that repository-identifying field, the receiver MUST return `400` (the body cannot identify a repository, FR-007); if the field is present but no enabled registration matches, the receiver MUST return `404` and write nothing. (A recognized non-`push` event is acknowledged with `202` before this resolution — see FR-007.)
...
- **FR-007**: On a handled delivery the receiver MUST return `202` with `{ data: { received, ingested, skipped, project } }` (a `ping` or recognized non-`push` event — any present, non-empty provider event header other than the push event — returns `202` with zero counts). A body that is not parseable JSON, that carries no provider event header (or an empty one), or that — for a `push` — lacks the repository-identifying field, MUST return `400`; nothing is written.
...
- **SC-003**: A delivery with a wrong/missing signature on a *registered* repo writes nothing and returns `401`; an unregistered repo returns `404`; an unsupported provider returns `404`; a disabled registration returns `404` — verified by a follow-up read showing nothing persisted. The `401`-vs-`404` split reveals only whether a (known-named) repo is registered — never a secret, a commit, or any other owner data (accepted trade-off, research D-7).
