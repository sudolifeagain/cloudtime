...
- [ ] **T-104**: `src/routes/webhooks.ts` — (a) owner CRUD `Hono<AuthEnv>` under `authMiddleware` mirroring `/ai/prices` (create/list/read/patch/delete; secret write-only; immutable provider/repo `400`; duplicate `409`; unknown/cross-user `404`; `401` before body); (b) public receiver `Hono<{ Bindings: Env }>` (no auth): raw-body read → parse (unparseable → `400`) → provider `404` → event (missing/empty header → `400`; `ping`/recognized non-push → `202` zero) → `extractRepo` (absent → `400`) → `lookupEndpoint` (none/disabled → `404`) → `verify` (fail → `401`) → `mapCommits` (cap 100, `skipped`) → one `db.batch()` of `commitUpsertStmt(db, reg.user_id, reg.project, v, v.total_seconds)` → `202 { data: {received,ingested,skipped,project} }`. Export `commitUpsertStmt` (+ the `CommitRow` type) from `commits.ts` for the receiver to reuse (behavior-preserving; `rowToCommit`/`UPSERT_SQL` are not reused — the ack is count-only). **No correlation query** (FR-005/FR-011).
