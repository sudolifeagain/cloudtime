...
**Independent Test**: In one project, commit A at `T0`, keep coding, commit B at `T1`; each posted without `total_seconds`. B's derived time covers only the coding since commit A (B's window lower bound is A's `author_date`, `T0`); A's and B's derived seconds sum to the session's active time, not double it. A heartbeat gap longer than the user's timeout inside a window is not counted.

**Acceptance Scenarios**:

1. **Given** a previous commit on the same project at `T0` and this commit's `author_date` at `T1`, **When** correlating, **Then** the attribution window is `[T0, T1]` (lower bound `T0`, upper bound inclusive); the timeline is partitioned so the coding since `T0` is attributed to this commit only — the heartbeat at the shared boundary `T0` is the previous commit's last beat and this commit's first, so no interval is double-counted.
...
- **FR-003**: The attribution window MUST be `[lower, upper]` (upper-**inclusive**) where `upper` is the commit's `author_date` (or the ingest instant when `author_date` is omitted) and `lower` is `max(previous_commit_author_date, upper − MAX_CORRELATION_WINDOW)`, with `previous_commit_author_date` = the greatest `author_date` strictly before `upper` among the same user's other commits in the same project (a null-`author_date` prior commit is not eligible). The upper bound is inclusive so a heartbeat at exactly `author_date` is counted, capturing the coding interval that ends at the commit moment (parity with `computeDurations`).
...
- **FR-008**: Consecutive commits MUST NOT double-count: the previous-commit lower bound partitions the timeline so the coding since the previous commit is attributed to `T_this`'s commit only. Because `T_this`'s upper bound is inclusive and equals the next commit's lower bound, a heartbeat on a shared commit boundary is the earlier commit's last beat and the later commit's first — the interval ending at the boundary counts for the earlier commit, the interval starting at it for the later one, so no interval is counted twice.
