...
3. **Given** the commit's project has heartbeats but a **different** project also has heartbeats in the same interval, **When** correlating, **Then** only the commit's project's heartbeats contribute (project-scoped, research D-5).
...
- **`author_date` omitted** → the window ends at the ingest instant (`now`, matching the read-path `created_at` fallback) and has no previous-commit partition — a capped-window best-effort correlation.
...
- **FR-004**: Only heartbeats with `project` equal to the commit's project and `time` within `[lower, upper)` MUST contribute. Correlation MUST NOT filter on branch/`ref` (research D-5).
- **FR-005**: The derived seconds MUST be the sum of consecutive in-window heartbeat gaps where `0 < gap <= user.timeout`, using the **same** session gap/timeout rule as the daily `summaries` aggregation, so a commit's derived time matches how `summaries` counts the same interval.
...
- **`heartbeats`** (existing, read-only here): correlated by `(user_id, project, time)`; `time` is Unix epoch seconds. Served by the existing `idx_heartbeats_user_time` index within the capped window.
...
- **SC-005**: An idle gap longer than the user's timeout inside a window contributes zero seconds; the derived total equals the `summaries` total for the same project/interval.
