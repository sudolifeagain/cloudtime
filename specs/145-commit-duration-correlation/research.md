...
3. one `SELECT time, project FROM heartbeats WHERE user_id = ? AND time >= ? AND time < ? ORDER BY time ASC LIMIT CORRELATION_HEARTBEAT_LIMIT` (the full in-window user stream — **not** project-filtered — so gaps can be attributed by `prev.project` per D-4);
