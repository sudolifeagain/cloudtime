...
**After** (excerpt):
> `hash` is required and must be non-empty. `total_seconds` is the optional coding time for the commit; when present it must be a number >= 0 and is stored verbatim. **When it is omitted, the server derives the commit's coding time from the user's heartbeats in a bounded window ending at `author_date` (falling back to the ingest time) and bounded below by the previous commit, counting only the coding time attributed to this project and excluding idle gaps beyond the session timeout (the same rule the daily summaries use); if no heartbeats contribute the value stays absent.** …
