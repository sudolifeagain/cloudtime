...
- [ ] **T-102**: `src/utils/commit-correlation.ts` (new) — pure helpers: `resolveWindow(upperEpoch, prevEpoch, maxWindow)` → `lowerEpoch`, and `sumActiveSeconds(sortedTimes, timeoutSec)` (uses `sessionGapSeconds`, rounds). No D1.
- [ ] **T-103**: `src/routes/commits.ts` — when the validated `total_seconds` is null, correlate before the upsert: resolve `upperText`/`upperEpoch` (`author_date ?? datetime('now')`, epoch via `strftime('%s', …)`), previous-commit `prevEpoch` (`MAX(author_date) … hash != ?`), user `timeout`, window heartbeat `time`s (`ORDER BY time ASC LIMIT 5000`), `sumActiveSeconds`, store `> 0 ? rounded : null`. Present value → unchanged single upsert. Update the file header.
...
- [ ] **T-105a**: Unit tests `tests/aggregation/commit-correlation.test.ts` — `sessionGapSeconds` (idle/out-of-order/exact-timeout boundary), `resolveWindow` (prev-bound vs cap floor), `sumActiveSeconds` (single beat → 0, idle-gap exclusion, rounding).
- [ ] **T-105b**: Integration tests `tests/integration/commits.test.ts` (extend) — derive-on-omit (US1), client-value-wins + no-query (US2), consecutive split / no double-count (US3), idle-gap exclusion equals summaries, no-heartbeats → absent/`"0 secs"`, explicit `0` honored, re-post re-derives, other-project heartbeats excluded, `author_date` omitted → capped window to now.
