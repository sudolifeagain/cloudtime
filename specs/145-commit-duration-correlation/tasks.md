...
- [ ] **T-103**: `src/routes/commits.ts` — when the validated `total_seconds` is null, correlate before the upsert: resolve `upperText`/`upperEpoch` (`author_date ?? datetime('now')`, epoch via `strftime('%s', …)`), previous-commit `prevEpoch` (`MAX(author_date) … hash != ?`), user `timeout`, window heartbeat `time`+`project`s (`WHERE user_id = ? AND time >= ? AND time < ?` — **not** project-filtered — `ORDER BY time ASC LIMIT 5000`), `sumActiveSeconds(rows, pathProject, timeoutSec)`, store `> 0 ? rounded : null`. Present value → unchanged single upsert. Update the file header.
