...
- [x] Session-boundary rule fixed: sum gaps `0 < gap <= timeout`, same as `summaries`, via a shared primitive — FR-005, research D-4.
- [x] Scope fixed: project-only, not branch/`ref` — FR-004, research D-5.
...
- [x] Derived time reconciles with `summaries` (same gap/timeout rule, shared helper) — FR-005, SC-005.
...
- [x] Quickstart scenarios A–H cover derive, client-wins, no-heartbeats, consecutive-split, idle-exclusion, re-post re-derive, explicit-zero, boundedness.
