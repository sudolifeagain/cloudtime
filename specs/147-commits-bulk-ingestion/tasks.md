...
- [x] **T-101**: `src/routes/commits.ts` — add the `POST /projects/:project/commits.bulk` handler under `authMiddleware`, mirroring `external_durations.bulk`: parse JSON → reject non-array (`400`) / over-100 (`400`) → validate every element with the existing `validateCommitInput` (`400 item {i}: {message}` on first failure, nothing written) → empty array → `201 { data: [] }` → one `db.batch()` of the upsert bound per element → shape via `rowToCommit` → `201 { data: Commit[] }`. Extract a shared `commitUpsertStmt(db, userId, project, v)` (behavior-preserving; the single POST binds it too) and reuse the existing `UPSERT_SQL`/`SELECT_COLUMNS`/`rowToCommit`. Register the route + its `authMiddleware` line. File header updated. **No correlation query in the bulk path** (FR-005, FR-011). *(Impl note: `commitUpsertStmt` takes an explicit `totalSeconds` arg so the single POST can pass its correlated/supplied value and the bulk path the verbatim value; the array-level guards live in a pure `validateCommitInputBatch(inputs, maxItems)` in `commit-input.ts` — reusing `validateCommitInput` per element — so T-102a can unit-test them without D1.)*
- [x] **T-102a**: Unit tests `tests/aggregation/commit-input.test.ts` (extend) — assert the array-level guards a bulk handler relies on: non-array rejected, over-cap rejected, first-invalid-element index reported, empty array yields no statements. (Keep pure — no D1.)
- [x] **T-102b**: Integration tests `tests/integration/commits.test.ts` (extend) — bulk create + read-back (order preserved), all-or-nothing 400 (mid-batch bad element leaves nothing written), non-array 400, over-100 400, empty array `201 { data: [] }`, omitted `total_seconds` → `"0 secs"` with **no** correlation even when heartbeats surround the commit, idempotent re-post across batches (single row), in-batch duplicate `(project, hash)` → response returns N entries in request order (earlier = pre-overwrite snapshot, later = last-wins) while a follow-up read returns exactly one last-wins row, `project` from path, cross-user isolation, 401 before validation, aggregates (`summaries`/`user_projects`) untouched.
- [x] **T-103**: `npm run typecheck` — zero errors.
- [x] **T-104**: `npm test` — full suite green incl. new unit + integration.
- [x] **T-105**: Commit with `feat:` prefix, push, open PR against `develop` referencing PR1 and issue #147 (`Closes #147`).
