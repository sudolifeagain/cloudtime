# Research: Heartbeat Spec WakaTime-CLI Compatibility
...
**Rationale**: wakatime-cli parses bulk responses by extracting `data.id` from each item. The current `[Heartbeat, code]` format returns the full heartbeat object, which the CLI does not expect. Returning only `{id}` in `data` minimizes payload size while satisfying CLI parsing.
...
**Rationale**: Older wakatime-cli versions and some plugins may send comma-separated strings. Accepting both formats ensures backward compatibility without data loss. DB column is TEXT, storing JSON-serialized arrays regardless of input format.
...
**Rationale**: This matches WakaTime/Wakapi behavior. Heartbeats within a 15-minute window are considered part of the same coding session. Computing at query time avoids extra storage and stays consistent as new heartbeats arrive.
...
**Rationale**: The body is the explicit, intentional value from wakatime-cli. The header may be set by HTTP libraries or proxies. Body-wins ensures the CLI's intended value is stored.
