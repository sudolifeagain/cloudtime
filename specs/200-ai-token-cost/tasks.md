...
- [x] T109 [US2] Extend `src/cron/aggregate.ts` to incrementally build the `ai_daily_usage` rollup: resolve `provider`/`model`/`agent`, sum token classes per `(day, provider, model, agent, project)` via `db.batch()`, watermark-driven, keying each bucket's `day` by the **token heartbeat's own** `time` (not `prev.time` — that is the summaries duration-interval key and would misattribute a token heartbeat across midnight). Coalesce nullable `project` to a `''` sentinel before the UPSERT (as the existing summaries pass does) so the unique index dedups exactly. Build a bucket only for *contributing* heartbeats (`category = 'ai coding'` with ≥1 priced token field present); skip prompt-length-only heartbeats. **Sum only `newHeartbeats` (`time > watermark`), never the prepended `lookbackHeartbeats`, or lookback tokens are re-added every run. Extend the heartbeat fetch SELECT to read the `ai_*` columns and `user_agent_id`, and resolve the `agent` label via `LEFT JOIN user_agents` (or a batched lookup on the distinct `user_agent_id`s) — the heartbeat row carries only the FK, not the label. Enqueue the `ai_daily_usage` UPSERTs in the SAME `db.batch()` as the `last_aggregated_at` advance so a mid-run failure cannot double-count or drop heartbeats.**
- [x] T110 [US2] Implement effective-price selection (owner-over-default precedence) and aggregate-then-price cost calculation in `src/utils/ai/pricing.ts`.
- [x] T111 [US2] Implement rollup-based daily + dimensional aggregation builders, single-currency selection, and cross-currency exclusion in `src/utils/ai/usage.ts`.
- [x] T112 [US2] Implement `GET /users/current/ai/usage` reading the `ai_daily_usage` rollup with deterministic range resolution (start+end vs days, `start>end`/one-sided/`>366d`/bad-timezone `400`), the `timezone` used only for range/"today" resolution and labeling (never re-bucketing stored days, FR-008), and owner-only access; mount the `ai` router in `src/index.ts`. Security seed: the usage/price routes inherit the API-wide `apiKeyQuery` scheme; prefer header/bearer auth on these owner-private cost reads and scrub `api_key` from access logs/Referer so credentials do not leak.
- [x] T113 [US2] Integration tests for token trends, estimated cost with/without prices (`null` + `missing_price_count`), single-currency selection + `mixed_currency` exclusion, range resolution `400`s, and owner-only `401` in `tests/integration/ai-usage.test.ts`; rollup aggregation tests in `tests/aggregation/ai-rollup.test.ts`.
