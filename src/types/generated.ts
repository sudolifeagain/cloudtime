...
export interface paths {
    ...
    "/users/current/projects/{project}/commits": {
         ...
         *     time from the user's heartbeats in a bounded window ending at `author_date`
         *     (falling back to the ingest time) and bounded below by the previous commit,
         *     counting only the coding time attributed to this project and excluding idle
         *     gaps beyond the session timeout (the same rule the daily summaries use); if
         *     no heartbeats contribute the value stays absent.
    ...
    "/webhooks/git/{provider}": {
         ...
         *
         *     A `ping` or a recognized non-`push` event is acknowledged with `202` and
         *     zero counts. Verification failure returns `401`; an unregistered or disabled
         *     repository, or an unsupported `{provider}`, returns `404`; an unparseable
         *     body returns `400`. Nothing is written unless the signature verifies.
         */
